...
public class ProximityProspectingPick : ModSystem {
    ...
    public override void AssetsLoaded(ICoreAPI api) {
        ...
        ObjectCacheUtil.GetOrCreate(api, ToolModesKey, () => GetSkillItems(api).ToArray());
    }
    ...
    private static SkillItem GetSkillItem(ICoreClientAPI? capi, string code, string name, string texturePath) {
        return new SkillItem() {
            ...
            it => {
                it.WithIcon(capi, capi?.Gui?.LoadSvgWithPadding(
                    loc: new AssetLocation(texturePath),
                    textureWidth: 48,
                    textureHeight: 48,
                    padding: 5,
